...
import {
  downloadPivotTable,
  listRegionPowerInfluenceProjects,
...
class InfluencePage extends React.Component<InfluencePageProps, any> {
  constructor(props: InfluencePageProps) {
    super(props);
  ...
  componentDidMount() {
    this.props.listProjects().then((projects) => {
      this.setState({
  ...
  projectsSelectDropdownChange(visible: boolean): void {
    ...
    if (!visible) {
      const ownerRepos = InfluencePage.projectOptions2ReqBody(this.state.selectedProjects);
      this.props.influenceOfProjects(ownerRepos).then((result) => {
        ...
        for (const item of result) {
          ...
          if (chartMaxVals.hasOwnProperty(key)) {
            if (chartMaxVals[key] < item[influenceDataKey]) {
              chartMaxVals[key] = item[influenceDataKey];
            }
          } else {
            chartMaxVals[key] = item[influenceDataKey];
          }
  ...
  genAndDownloadPivotTable() {
    ...
    const ownerRepos = InfluencePage.projectOptions2ReqBody(this.state.selectedProjects);
    this.props.influencePivotTable(ownerRepos).then((response) => {
      ...
      <div>
        ...
        <Row>
          {this.state.chartDataList.map((chartData, index) => {
            // TODO update the key by derived class names
            return (
              <Col span={12} key={`region_influence_chart__${index}`}>
...
export default class RegionPowerInfluence extends React.Component<any, any> {
  ...
  componentDidMount() {
    listRegionPowerInfluenceProjects().then((projects) => {
      this.setState({
        projectOptions: projects.map((p) => {
          const { owner, repo } = p;
          return {
  ...
  projectsSelectDropdownChange(visible: boolean): void {
    ...
    if (!visible) {
      const ownerRepos = RegionPowerInfluence.projectOptions2ReqBody(this.state.selectedProjects);
